...
public class MergeSort {
    ...
    public MergeSort(String arqE, String arqS) {
        this.arq = arqE;
        this.arq2 = arqS;
    }
    ...
    private void salvarTempo(int n, double timeTaken) {
        try (PrintWriter writer = new PrintWriter(new FileWriter(this.arq2, true))) {
            writer.print("\nJava," + n + "," + String.format(Locale.US, "%.10f", timeTaken) + "," + arq);
            System.out.println("\nTempo de execução: " + String.format(Locale.US, "%.10f", timeTaken));    
        } catch (IOException e) {
            System.err.println("Erro ao salvar o tempo de execução: " + e.getMessage());
        }
    }
